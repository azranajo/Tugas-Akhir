...
# Fungsi ketika tombol "exit" ditekan
def exit_program():
    global captured
    captured = True
    root.destroy()
...
btn_capture = tk.Button(button_frame, text="Capture", font=("Arial", 18), bg="green", fg="white", command=capture_image)
btn_capture.pack(side="left", padx=20)

btn_exit = tk.Button(button_frame, text="Exit", font=("Arial", 18), bg="red", fg="white", command=exit_program)
btn_exit.pack(side="left", padx=20)

# Jalankan preview kamera di thread
